...
class OrderService {
    ...
    async findAllOrder() {
        try {
            const allOrder = await order_1.OrderModel.find({}, { _id: 1, customerName: 1, products: 1, totalAmount: 1, createdAt: 1 })
                .sort({ createdAt: -1 })
                .exec();
    ...
    async getOrderByCustomer(customerClerkId) {
        try {
            const allOrderByCustomer = await order_1.OrderModel.find({ customerClerkId })
                .sort({ createdAt: -1 })
                .populate("products.product");
            return {
                code: 200,
